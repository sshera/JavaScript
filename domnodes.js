(function () {
    ...
    function updateUI() {
        ...
        // 2em means 2x size of current font
        ...
        for (i = 0; i < colors.length; i++) {
            //Creating a node dynamically with the intent of adding it to the colorDiv
            var node = document.createElement('div');
            var textnode = document.createTextNode(colors[i].name);
            node.appendChild(textnode);
            node.style.backgroundColor = colors[i].value;

            //Alternatively, you could make this into a CSS style and add it to the node.classList
